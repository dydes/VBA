...
'判断总分表表头
    If Sheets("总表").Range("A1") <> "学号" Then
        MsgBox "请勿更改总表表头：学号、班级、姓名、理科综合、物理、化学、生物"
        GoTo 1
    End If
...
'DEFG列转为数值格式
    For i = 1 To rowmax
        Sheets("总表").Range("H" & i) = Range("D" & i).Value
        Sheets("总表").Range("I" & i) = Range("E" & i).Value
        Sheets("总表").Range("J" & i) = Range("F" & i).Value
        Sheets("总表").Range("K" & i) = Range("G" & i).Value
    Next
...
'按班级升序，按总分降序
    ...
    ActiveWorkbook.Worksheets("总表").Sort.SortFields.Add Key:=Range("G2:G" & rowmax), _
        SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:=xlSortNormal
    With ActiveWorkbook.Worksheets("总表").Sort
        .SetRange Range("A1:G" & rowmax)
        .Header = xlYes
        .MatchCase = False
...
'HI列增加辅助公式(用来确定班级的起始结束位置)
    For i = 2 To rowmax
        Sheets("总表").Range("H" & i).Formula = "=IF(B" & i & "=B" & i - 1 & ","""",1)"
        If Sheets("总表").Range("H" & i) = 1 Then
            Sheets("总表").Range("I" & i) = i
        End If
    Next
...
'逐个创建各班总分sheet
    For i = 5 To rowmax2
        ...
        Sheets("总表").Range("B" & bg & ":G" & en & "").Copy
        Sheets(clName).Select
        Range("A2").Select
        ActiveSheet.Paste
...
'逐个创建各班双栏sheet
    For i = 5 To rowmax2
        ...
        Sheets("总表").Range("B" & bgl & ":G" & enl & "").Copy
        Sheets(clName).Select
        Range("A2").Select
        ActiveSheet.Paste
        Columns("A:A").ColumnWidth = 20
        Sheets("总表").Range("B" & bgr & ":G" & enr & "").Copy
        Sheets(clName).Select
        Range("H2").Select
        ActiveSheet.Paste
...
'逐个创建各班三栏sheet
    ...
    For i = 5 To rowmax2
        ...
        Cells.Select
        Cells.EntireRow.AutoFit
        Sheets("总表").Range("B" & bgm & ":G" & enm & "").Copy
        Sheets(clName).Select
        Range("H2").Select
        ActiveSheet.Paste
        Columns("H:H").ColumnWidth = 20
